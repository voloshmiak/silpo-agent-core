...
8. ПОБАЖАННЯ КОРИСТУВАЧА. Якщо в запиті є прямі побажання — улюблений продукт, страва,
   «люблю X», «хочу X» — вони мають бути або в плані, або пояснені в summary.notes
   (не вклалось у бюджет, порушує обмеження, немає в наявності). Мовчки проігнороване
   побажання — порушення.
...
def build_plan_prompt(
    *,
    ...
    if note:
        lines.append(
            f"Додатково від користувача: {note}. Це прямі побажання — врахуй їх у раціоні "
            "й кошику. Якщо якесь виконати не вдається (бюджет, обмеження, немає в "
            "наявності), напиши в summary.notes, яке саме і чому."
        )
    if previous_plan:
