...
8. БЮДЖЕТ. Виклич check_budget. Якщо перевищено — шукай дешевші аналоги через
   ...
   вкладешся. Білок ріжеш останнім. Числа з line_items іди прямо в кошик плану.
9. ПІДСУМКИ ПО ДНЯХ. Виклич sum_macros на прийомах їжі кожного дня, щоб отримати денні
   ккал і БЖВ, а потім check_plan_days на всіх семи днях. Якщо якийсь день поза
   допуском — виправ його раціон і перевір ще раз.
10. КОШИК. Перевір поточний стан через silpo_get_my_shopping_cart, потім додай товари
    через silpo_add_or_update_cart_products.
11. ЗАВЕРШЕННЯ. Виклич finalize_plan з повним планом. Це останній виклик інструмента —
...
- cart: фінальний кошик — назва, product_id, кількість, ціна за одиницю, сума;
